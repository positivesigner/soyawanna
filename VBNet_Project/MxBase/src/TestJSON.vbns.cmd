...
Namespace Mx
    Public Class UserAction
        ...
        Public Shared Sub Result(ur_fn As enfBG.zrun_tests, ret_msg As Strap, worker As System.ComponentModel.BackgroundWorker)
            ...
            For Each enfENTRY In test_list
                ...
                Dim setup_value_count = Assistant.Store_JSON_Setup(
                    message_written,
                    enfENTRY,
                ...
                message_written = Assistant.Test_JSON(
                    message_written,
                    ret_msg,
                    enfENTRY,
                    file_comment_bowlname,
    ...
    Public Class Assistant
        Public Class enfTEST
            Inherits bitBASE
            Public Shared json_root_array = TRow(Of enfTEST).glbl.NewBitBase()
            Public Shared json_root_row = TRow(Of enfTEST).glbl.NewBitBase()
            Public Shared json_root_string = TRow(Of enfTEST).glbl.NewBitBase()
            Public Shared json_root_string_quoted = TRow(Of enfTEST).glbl.NewBitBase()
            Public Shared json_skip_ws_preroot = TRow(Of enfTEST).glbl.NewBitBase()
            Public Shared json_unnamed_root_subarray = TRow(Of enfTEST).glbl.NewBitBase()
            Public Shared json_unnamed_root_subarray_quoted = TRow(Of enfTEST).glbl.NewBitBase()
            Public Shared json_include_value_subarray = TRow(Of enfTEST).glbl.NewBitBase()
            Public Shared json_include_value_subarray_quoted = TRow(Of enfTEST).glbl.NewBitBase()
        End Class

        Public Shared Function Store_JSON_Setup(
            ...
            If ur_message_written = False Then
                Store_JSON_Setup = 4
                If ur_fn Is enfTEST.json_root_array Then
                    Have.TempBowl.SelKey(ur_file_comment_bowlname).Contents = "There is a root array. There is only one row, which has one column."

                    Dim stpFILE = Strapd().d(
                    "[").d(
                    "{").d(
                    "'key_e1':'val_e1'".Replace(qt, qs)).d(
                    "}").d(
                    "]")
                    ...
                    "row_seq-key-value".Replace("-", vbTab)).dLine().d(
                    "'1'-'key_e1'-'val_e1'".Replace("-", vbTab).Replace(qt, qs)).dLine.dLine()
                    Have.TempBowl.SelKey(ur_exp_table_bowlname).Contents = stpEXPECTED_TABLE.ToString

                ElseIf ur_fn Is enfTEST.json_root_row Then
                    Have.TempBowl.SelKey(ur_file_comment_bowlname).Contents = "There is no root array. The root row has one column."
                ...
                ElseIf ur_fn Is enfTEST.json_root_string Then
                    Have.TempBowl.SelKey(ur_file_comment_bowlname).Contents = "There is a root array. There is one unnamed string and one row."

                    Dim stpFILE = Strapd().d(
                    "[").d(
                    "'unnamed value'".Replace(qt, qs)).d(
                    ",").d(
                    "{").d(
                    "'key_e1':'val_e1'".Replace(qt, qs)).d(
                    "}").d(
                    ...
                    Dim stpEXPECTED_TABLE = Strapd().d(
                    "row_seq-key-value".Replace("-", vbTab)).dLine().d(
                    "'1'-''-'unnamed value'".Replace("-", vbTab).Replace(qt, qs)).dLine.dLine().d(
                    "'2'-'key_e1'-'val_e1'".Replace("-", vbTab).Replace(qt, qs)).dLine.dLine()
                    Have.TempBowl.SelKey(ur_exp_table_bowlname).Contents = stpEXPECTED_TABLE.ToString

                ElseIf ur_fn Is enfTEST.json_skip_ws_preroot Then
                    Have.TempBowl.SelKey(ur_file_comment_bowlname).Contents = "There is a root array. There is whitespace before the root array."

                    Dim stpFILE = Strapd().d(s).d(s).dLine().dLine().d(vbTab).d(
                    "[").d(
                    "{").d(
                    "'key_e1':'val_e1'".Replace(qt, qs)).d(
                    "}").d(
                ...
                ElseIf ur_fn Is enfTEST.json_root_string_quoted Then
                    Have.TempBowl.SelKey(ur_file_comment_bowlname).Contents = "There is a root array. There is only one row, which has one column. The column name and the value both have an escaped quote character"

                    Dim stpFILE = Strapd().d(
                    "[").d(
                    "{").d(
                    "'key_e1\'cont_e1':'val_e2\'cont_e2'".Replace(qt, qs)).d(
                    "}").d(
                    "]")
                    Have.TempBowl.SelKey(ur_file_text_bowlname).Contents = stpFILE
                    ...
                    Dim stpEXPECTED_TABLE = Strapd().d(
                    "row_seq-key-value".Replace("-", vbTab)).dLine().d(
                    "'1'-'key_e1''cont_e1'-'val_e2''cont_e2'".Replace("-", vbTab).Replace(qt, qs)).dLine.dLine()
                    Have.TempBowl.SelKey(ur_exp_table_bowlname).Contents = stpEXPECTED_TABLE.ToString

                ElseIf ur_fn Is enfTEST.json_unnamed_root_subarray Then
                    Have.TempBowl.SelKey(ur_file_comment_bowlname).Contents = "There is a root array. There is a sub-array followed by one row."
                    ...
                    Dim stpEXPECTED_TABLE = Strapd().d(
                    "row_seq-key-value".Replace("-", vbTab)).dLine().d(
                    "'1'-''-'2345,6789'".Replace("-", vbTab).Replace(qt, qs)).dLine.dLine().d(
                    "'2'-'key_e1'-'val_e1'".Replace("-", vbTab).Replace(qt, qs)).dLine.dLine()
                    Have.TempBowl.SelKey(ur_exp_table_bowlname).Contents = stpEXPECTED_TABLE.ToString

                ElseIf ur_fn Is enfTEST.json_unnamed_root_subarray_quoted Then
                    Have.TempBowl.SelKey(ur_file_comment_bowlname).Contents = "There is a root array. There is a sub-array followed by one row. The sub-array has an escaped quote."

                    Dim stpFILE = Strapd().d(
                    "[").d(
                    "[").d(
                    "'val_e2\'cont_e2'".Replace(qt, qs)).d(
                    ",").d(
                    "6789").d(
                    "]").d(
                    ",").d(
                    ...
                    Dim stpEXPECTED_TABLE = Strapd().d(
                    "row_seq-key-value".Replace("-", vbTab)).dLine().d(
                    "'1'-''-'''val_e2\''cont_e2'',6789'".Replace("-", vbTab).Replace(qt, qs)).dLine.dLine().d(
                    "'2'-'key_e1'-'val_e1'".Replace("-", vbTab).Replace(qt, qs)).dLine.dLine()
                    Have.TempBowl.SelKey(ur_exp_table_bowlname).Contents = stpEXPECTED_TABLE.ToString

                ElseIf ur_fn Is enfTEST.json_include_value_subarray Then
                    Have.TempBowl.SelKey(ur_file_comment_bowlname).Contents = "There is a root array. There is one row with two columns. On column has a sub-array."

                    Dim stpFILE = Strapd().d(
                    "[").d(
                    "{").d(
                    "'key_e1':'val_e1',".Replace(qt, qs)).d(
                    "'key_e2':['val_e2','val_e3']".Replace(qt, qs)).d(
                    "}").d(
                    "]")
                    Have.TempBowl.SelKey(ur_file_text_bowlname).Contents = stpFILE
                    ...
                    Dim stpEXPECTED_TABLE = Strapd().d(
                    "row_seq-key-value".Replace("-", vbTab)).dLine().d(
                    "'1'-'key_e1'-'val_e1'".Replace("-", vbTab).Replace(qt, qs)).dLine.dLine().d(
                    "'2'-'key_e2'-'[''val_e2'',''val_e3'']'".Replace("-", vbTab).Replace(qt, qs)).dLine.dLine()
                    Have.TempBowl.SelKey(ur_exp_table_bowlname).Contents = stpEXPECTED_TABLE.ToString

                ElseIf ur_fn Is enfTEST.json_include_value_subarray_quoted Then
                    Have.TempBowl.SelKey(ur_file_comment_bowlname).Contents = "There is a root array. There is one row with one columns. The column has a sub-array."

                    Dim stpFILE = Strapd().d(
                    "[").d(
                    "{").d(
                    "'key_e1':['val_e1\'cont_e1','val_e2']".Replace(qt, qs)).d(
                    "}").d(
                    "]")
                    Have.TempBowl.SelKey(ur_file_text_bowlname).Contents = stpFILE
                    ...
                    Dim stpEXPECTED_TABLE = Strapd().d(
                    "row_seq-key-value".Replace("-", vbTab)).dLine().d(
                    "'1'-'key_e1'-'[''val_e1\''cont_e1'',''val_e2'']'".Replace("-", vbTab).Replace(qt, qs)).dLine.dLine()
                    Have.TempBowl.SelKey(ur_exp_table_bowlname).Contents = stpEXPECTED_TABLE.ToString
                ...
                End If 'ur_fn
            End If
        End Function 'Store_JSON_Setup

        Public Shared Function Test_JSON(
            ur_message_written As Boolean,
            ret_msg As Strap,
            ur_fn As enfTEST,
            ur_file_comment_bowlname As enkTB.zfile_comment,
            ...
            Test_JSON = ur_message_written
            If ur_message_written = False Then
                If ur_fn Is enfTEST.json_include_value_subarray_quoted Then
                    Dim abc = 123
                End If
                ...
                Dim stpERROR = Strapd()
                Dim sdaSHOULD = MxText.ParseJSON(strFILE, stpERROR)
                Dim stpOUTPUT_LINES = Strapd()
                For Each trwENTRY In sdaSHOULD.kvp
                    For Each colVAL In trwENTRY.row.kvp
                        stpOUTPUT_LINES.d(colVAL.row.ToString(trwENTRY.Indexb1 = 1 And colVAL.Indexb1 = 1))
                    Next
                Next trwENTRY
