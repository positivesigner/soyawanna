...
Namespace Mx
    Public Class UserAction
        ...
        Public Shared Sub Result(ur_fn As enfBG.zrun_tests, ret_msg As Strap, worker As System.ComponentModel.BackgroundWorker)
            ...
            For Each enfENTRY In test_list
                ...
                Dim setup_value_count = Assistant.Store_CSV_Setup(
                    message_written,
                    enfENTRY,
                ...
                message_written = Assistant.Test_CSV(
                    message_written,
                    ret_msg,
                    enfENTRY,
                    file_comment_bowlname,
    ...
    Public Class Assistant
        Public Class enfTEST
            Inherits bitBASE
            Public Shared unquoted_value = TRow(Of enfTEST).glbl.NewBitBase()
            Public Shared quoted_value = TRow(Of enfTEST).glbl.NewBitBase()
            Public Shared esc_quoted_value = TRow(Of enfTEST).glbl.NewBitBase()
            Public Shared two_value = TRow(Of enfTEST).glbl.NewBitBase()
        End Class

        Public Shared Function Store_CSV_Setup(
            ...
            If ur_message_written = False Then
                Store_CSV_Setup = 3
                If ur_fn Is enfTEST.unquoted_value Then
                    Have.TempBowl.SelKey(ur_file_comment_bowlname).Contents = "There is an unquoted value."

                    Dim stpFILE = Strapd().d(
                    "val_e1".Replace(qt, qs))
                    ...
                    "value").dLine().d(
                    "val_e1".Replace(qt, qs)).dLine
                    Have.TempBowl.SelKey(ur_exp_table_bowlname).Contents = stpEXPECTED_TABLE.ToString

                ElseIf ur_fn Is enfTEST.quoted_value Then
                    Have.TempBowl.SelKey(ur_file_comment_bowlname).Contents = "There is a quoted value."
                ...
                ElseIf ur_fn Is enfTEST.esc_quoted_value Then
                    Have.TempBowl.SelKey(ur_file_comment_bowlname).Contents = "There is an escaped quoted value."

                    Dim stpFILE = Strapd().d(
                    "'val_e1''cont_e1".Replace(qt, qs))
                    Have.TempBowl.SelKey(ur_file_text_bowlname).Contents = stpFILE
                    ...
                    Dim stpEXPECTED_TABLE = Strapd().d(
                    "value").dLine().d(
                    "val_e1'cont_e1".Replace(qt, qs)).dLine
                    Have.TempBowl.SelKey(ur_exp_table_bowlname).Contents = stpEXPECTED_TABLE.ToString

                ElseIf ur_fn Is enfTEST.two_value Then
                    Have.TempBowl.SelKey(ur_file_comment_bowlname).Contents = "There are two values."

                    Dim stpFILE = Strapd().d(
                    "val_e1,val_e2".Replace(qt, qs))
                    Have.TempBowl.SelKey(ur_file_text_bowlname).Contents = stpFILE
                    ...
                    Dim stpEXPECTED_TABLE = Strapd().d(
                    "value").dLine().d(
                    "val_e1".Replace(qt, qs)).dLine.d(
                    "val_e2".Replace(qt, qs)).dLine
                    Have.TempBowl.SelKey(ur_exp_table_bowlname).Contents = stpEXPECTED_TABLE.ToString
                ...
                End If 'ur_fn
            End If
        End Function 'Store_CSV_Setup

        Public Shared Function Test_CSV(
            ur_message_written As Boolean,
            ret_msg As Strap,
            ur_fn As enfTEST,
            ur_file_comment_bowlname As enkTB.zfile_comment,
            ...
            If ur_message_written = False Then
                ...
                Dim stpERROR = Strapd()
                Dim sdaSHOULD = MxText.CSVSplit(strFILE, stpERROR)
                Dim stpOUTPUT_LINES = Strapd().d("value").dLine
                For Each trwENTRY In sdaSHOULD.kvp
                    stpOUTPUT_LINES.d(trwENTRY.v).dLine()
                Next trwENTRY
