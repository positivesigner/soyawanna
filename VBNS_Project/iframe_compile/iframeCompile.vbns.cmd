...
Namespace Mx
    Public Class Want
        Public Shared Sub TWCompare_ExportFiles(ret_message As Strap)
            ...
            Dim rootfolder_bowlname = enmUN.app_folder
			Dim rootindex_bowlname = enmUN.root_index
			userbowl_cart.SelKey(rootindex_bowlname).v(enmUB.contents) = "index.html"
            Dim tf_txt_html = tempfile_cart.Apply(rootfolder_bowlname, windows_fs_cart)
            subdir_cart.Apply(tf_txt_html, rootfolder_bowlname, tempfile_cart)
            Dim tf_folder_remove = tempfile_cart.Apply(rootfolder_bowlname, subdir_cart, windows_fs_cart)
            Dim tf_key_list = tempfile_cart.Apply(rootfolder_bowlname, tf_txt_html, windows_fs_cart)
            Dim tf_missing_txthtmlpath = tf_key_list.missing_txthtmlpath
            Dim tf_misspelled_srctxtpath = tf_key_list.misspelled_srctxtpath
            windows_fs_cart.Apply(rootfolder_bowlname, rootindex_bowlname, subdir_cart)
    ...
    Public Class Have
        ...
        Partial Class sSubDir
            Public Sub Apply(ur_tf_txt_html As enmTN.ztxt_html, ur_in_folder As enmUN.zapp_folder, ur_tempfile As Have.sTempFile)
        ...
        Partial Class sTempFile
            Public Function Apply(ur_root_folder As enmUN.zapp_folder, ur_windows_fs_cart As Have.sWindowsFS) As enmTN.ztxt_html
            ...
            Public Function Apply(ur_root_folder As enmUN.zapp_folder, ur_subdir_cart As Have.sSubDir, ur_windows_fs_cart As Have.sWindowsFS) As enmTN.zremove_indexpath
                ...
                For Each rowPATH In ur_windows_fs_cart.InsList_From_Windows(strROOT_FOLDER, FileNamed().d("*").dAppendEXT(lit_subdir_html), System.IO.SearchOption.AllDirectories).SelAll
                    Dim flnFILE = FileNamed().d(rowPATH.v(enmWF.file_path))
                    If ur_subdir_cart.SelEachSection.Contains(flnFILE) = False Then
                        Me.Ins(
                            New Have.rTempFile().
                            vt(enmTF.section, retKEY.name).
                            vt(enmTF.in_filepath, flnFILE)
            ...
            Public Function Apply(ur_root_folder As enmUN.zapp_folder, ur_subdir_cart As enmTN.ztxt_html, ur_windows_fs_cart As Have.sWindowsFS) As tf_keylist_missing_misspelled
                ...
                Apply = retKEY_LIST
                Dim strROOT_FOLDER = Have.UserBowl.SelKey(ur_root_folder).v(enmUB.contents)
                'sample1 C:\...\subdir\...\Windows\t1.src.txt
                For Each rowPATH In ur_windows_fs_cart.InsList_From_Windows(strROOT_FOLDER, FileNamed().d("*").dAppendEXT(lit_src_txt), System.IO.SearchOption.AllDirectories).SelAll
                    Dim flnFILE = FileNamed().d(rowPATH.v(enmWF.file_path))
                    If glbl.gWindowsFS.HasFile(flnFILE.gParentDir.d(flnFILE.FileBaseGroup).dAppendEXT(lit_txt_html)) = False Then
                        Me.Ins(
                            New Have.rTempFile().
        ...
        Partial Public Class sWindowsFS
            Public Sub Apply(ur_root_folder As enmUN.zapp_folder, ur_root_index As enmUN.zroot_index, ur_subdir As Have.sSubDir)
    ...
    Public Class enmUN
        ...
        Public Shared root_index As zroot_index = TRow(Of enmUN).glbl.Trbase(Of zroot_index).NewBitBase() : Public Class zroot_index : Inherits enmUN : End Class
    End Class
